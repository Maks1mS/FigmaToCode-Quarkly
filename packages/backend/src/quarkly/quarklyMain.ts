...
export const quarklyMain = (
  ...
  let result = htmlWidgetGenerator(sceneNode, true);
...
const htmlGroup = (node: GroupNode, isJsx: boolean = false): string => {
  ...
  if (builder.styles) {
    ...
    return `\n<Box${attr}>${indentString(generator)}\n</Box>`;
  }
...
export const htmlText = (node: TextNode, isJsx: boolean): string => {
  ...
  return `\n<Text${layoutBuilder.build()}>${content}</Text>`;
};
